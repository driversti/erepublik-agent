...
describe('formatBattleSuccessMessage', () => {
  it('renders battle id, deep-link with battleZoneId, both flags, division', () => {
    // `#` is reserved in MarkdownV2 and must be escaped inside link text.
  ...
  it('escapes MarkdownV2-reserved chars in region names (parens, dots, dashes)', () => {
    const msg = formatBattleSuccessMessage({ ...sample, regionName: 'Northern Region (East)' });
    expect(msg).toContain(
      '[\\#12345 Northern Region \\(East\\)](https://www.erepublik.com/en/military/battlefield/12345/9876)',
    );
  });
...
describe('formatBattleFailureMessage', () => {
  it('includes the same header plus the escaped reason on a new line', () => {
    ...
    // The `-` in `travel-b` is escaped under MarkdownV2.
